<?php

declare(strict_types=1);

use Arqel\Tenant\Commands\ScaffoldBillingCommand;
use Illuminate\Filesystem\Filesystem;
use Symfony\Component\Console\Input\ArrayInput;
use Symfony\Component\Console\Output\BufferedOutput;

/**
 * Drive the command via its `setBasePath()` testing hook so we
 * never touch the host's `app/` or `resources/` folders.
 */
function runScaffoldBillingCommand(string $basePath, bool $force = false): int
{
    $command = new ScaffoldBillingCommand;
    $command->setBasePath($basePath);
    $command->setLaravel(app());

    $input = new ArrayInput($force ? ['--force' => true] : []);
    $output = new BufferedOutput;

    return $command->run($input, $output);
}

beforeEach(function (): void {
    $this->tempBase = sys_get_temp_dir().'/arqel-tenant-billing-scaffold-'.uniqid();
    mkdir($this->tempBase, 0o755, true);

    config()->set('arqel.tenancy.model', 'App\\Models\\Tenant');
});

afterEach(function (): void {
    $files = new Filesystem;
    if (is_dir($this->tempBase)) {
        $files->deleteDirectory($this->tempBase);
    }
});

it('writes the billing controller, routes snippet, and React stub on a clean run', function (): void {
    $exit = runScaffoldBillingCommand($this->tempBase);

    expect($exit)->toBe(0);

    $controller = $this->tempBase.'/app/Http/Controllers/BillingController.php';
    $routes = $this->tempBase.'/routes/billing.php';
    $page = $this->tempBase.'/resources/js/Pages/Arqel/Billing.tsx';

    expect(file_exists($controller))->toBeTrue();
    expect(file_exists($routes))->toBeTrue();
    expect(file_exists($page))->toBeTrue();

    $controllerContents = (string) file_get_contents($controller);
    expect($controllerContents)->toContain('namespace App\\Http\\Controllers;');
    expect($controllerContents)->toContain('use App\\Models\\Tenant;');
    expect($controllerContents)->toContain("Inertia::render('Arqel/Billing'");
    expect($controllerContents)->toContain('Cashier');

    $routesContents = (string) file_get_contents($routes);
    expect($routesContents)->toContain('tenant.billing.show');
});

it('is idempotent — second run without --force leaves files untouched and exits 0', function (): void {
    runScaffoldBillingCommand($this->tempBase);

    $controller = $this->tempBase.'/app/Http/Controllers/BillingController.php';
    file_put_contents($controller, '// hand-edited');

    $exit = runScaffoldBillingCommand($this->tempBase);

    expect($exit)->toBe(0);
    expect((string) file_get_contents($controller))->toBe('// hand-edited');
});

it('overwrites existing files when --force is passed', function (): void {
    runScaffoldBillingCommand($this->tempBase);

    $controller = $this->tempBase.'/app/Http/Controllers/BillingController.php';
    file_put_contents($controller, '// hand-edited');

    $exit = runScaffoldBillingCommand($this->tempBase, force: true);

    expect($exit)->toBe(0);
    expect((string) file_get_contents($controller))->toContain('BillingController');
});

it('appends the routes snippet to routes/web.php when present and skips on re-run', function (): void {
    mkdir($this->tempBase.'/routes', 0o755, true);
    file_put_contents($this->tempBase.'/routes/web.php', "<?php\n\n// existing routes\n");

    runScaffoldBillingCommand($this->tempBase);

    $webRoutes = (string) file_get_contents($this->tempBase.'/routes/web.php');
    expect($webRoutes)->toContain('tenant.billing.show');

    runScaffoldBillingCommand($this->tempBase);

    $afterSecond = (string) file_get_contents($this->tempBase.'/routes/web.php');
    expect(substr_count($afterSecond, 'tenant.billing.show'))->toBe(1);
});
